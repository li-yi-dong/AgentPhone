...
enum MessageType {
  ...
  // 注意：PTY 实际尺寸由本地终端 SIGWINCH 唯一决定，手机端不得修改 PTY 尺寸
  MOBILE_VIEWPORT = 'mobile:viewport',   // Mobile → Desktop（手机视口声明，仅用于客户端渲染，不影响 PTY）

  // 连接握手（challenge-response，token 不在网络中明文传输）
  AUTH_CHALLENGE = 'auth:challenge',     // Desktop → Mobile（Desktop 发送随机 challenge）
  AUTH_RESPONSE  = 'auth:response',      // Mobile → Desktop（HMAC-SHA256(token, challenge)）
...
Desktop Agent 启动时生成一个随机 `token`（≥128 bit CSPRNG，Base62 编码），显示在终端二维码中。认证使用 **challenge-response**，token 本身不在网络中传输：
...
用户终端              Desktop Agent              手机 APP
  ...
  │◄──显示 IP + 二维码──── │                        │
  │   ws://192.168.1.5:8080│                        │
  │   token=<base62>       │（token 仅在本地显示）  │
  │                        │                        │
  │                        │◄──WS Connect──────────│
  │                        │──AUTH_CHALLENGE───────►│（Desktop 发随机 challenge）
  │                        │◄──AUTH_RESPONSE───────│（HMAC-SHA256(token, challenge)）
  │                        │   验证 HMAC            │
  │                        │──AUTH_OK─────────────►│
  │                        │──STATE_SNAPSHOT───────►│（历史缓冲）
  │                        │                        │
...
// 手机视口声明（Mobile → Desktop，手机横竖屏切换时发送）
// 注意：仅通知手机端 xterm.js 调整渲染视口，不修改 PTY 实际尺寸
{ type: 'mobile:viewport', timestamp: 1721665893211,
...
- **token 生成**：每次启动随机生成，≥128 bit CSPRNG 随机字节，Base62 编码，仅在本地终端显示，不在网络中明文传输
- **认证机制**：challenge-response（HMAC-SHA256），攻击者截获网络流量无法获取 token
- **token 生命周期**：Desktop Agent 重启即失效；同一 token 在运行期间可用于断线重连（会话级 token，非"一次性"）
- **传输安全**：MVP 使用 `ws://`（局域网内，威胁模型已知）；后续升级为 `wss://` 自签名证书
- **PTY Resize 权威源**：PTY 实际尺寸由本地终端 SIGWINCH 信号唯一决定，手机端发送 `MOBILE_VIEWPORT` 仅影响其自身的 xterm.js 渲染视口
- **输入串行化**：本地终端和手机端输入均先到先处理，顺序写入 PTY，互不干扰
...
| 2026-07-22 | v0.4.0 | 修复 P0 设计问题：PTY Resize 权威源改为本地终端唯一决定，手机端改用 MOBILE_VIEWPORT；认证改为 challenge-response（HMAC-SHA256），token 不在网络明文传输 | Manager Agent |
